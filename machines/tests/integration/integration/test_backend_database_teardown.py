...
def test_backend_database_relation_removal(juju: Juju, charm: str, ubuntu_base: str) -> None:
    ...
    # The router is a subordinate of the client application (mysql-test-app).
    juju.integrate(
        f"{MYSQL_TEST_APP_NAME}:database",
        f"{MYSQL_ROUTER_APP_NAME}:database",
    )

    logging.info("Waiting for applications to become active")
    juju.wait(
    ...
    logging.info("Waiting for mysql-router to block on the missing backend-database relation")
    juju.wait(
    ...
    logging.info("Re-integrating mysql with mysql-router")
    juju.integrate(
        f"{MYSQL_SERVER_APP_NAME}:database",
        f"{MYSQL_ROUTER_APP_NAME}:backend-database",
    )
    ...
    logging.info("Waiting for applications to become active again")
    juju.wait(
        ready=wait_for_apps_status(jubilant_backports.all_active),
        error=jubilant_backports.any_error,
        timeout=10 * MINUTE_SECS,
